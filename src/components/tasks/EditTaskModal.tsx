...
export default function EditTaskModal({data, taskId}: EditTaskModalProps) {
    ...
    return (
        <Transition appear show={true} as={Fragment}>
            <Dialog as="div" className="relative z-10" onClose={() => navigate(location.pathname, {replace : true}) }> {/* Close the modal when clicking outside the modal */}
                <Transition.Child
                    as={Fragment}
                    ...
                    <div className="flex min-h-full items-center justify-center p-4 text-center">
                        ...
                        >
                            <Dialog.Panel className="w-full max-w-4xl transform overflow-hidden rounded-2xl bg-white text-left align-middle shadow-xl transition-all p-6">
                                <Dialog.Title
                                    as="h3"
                                    className="text-2xl my-5 text-center font-black"
                                >
                                    {/* Editar Tarea */}
                                </Dialog.Title>

                                <p className="text-xl font-bold">Editar la {''}
                                    <span className="text-sky-500">tarea</span>
                                </p>
